  ...
  if (!uri || [uri isEqualToString:@""]) {
    ...
    DebugLog(@"Could not find video URL in source '%@'", source);
      if (self.onVideoError) {
          self.onVideoError(@{@"error": @{@"code": @"1000",
                                          @"localizedDescription": @"Could not find video URL in source",
                                          @"localizedFailureReason": @"Could not find video URL in source",
                                          @"localizedRecoverySuggestion": @"Could not find video URL in source",
                                          @"domain": @"RTCVideo"},
                              @"target": @"ABA"});
      }
    return;
